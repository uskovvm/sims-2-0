  ...
  return React.createClass({
    ...
    render: function () {
      ...
      if (self.state.content && self.state.content.length > 0) {
        let data = self.state.content.map(function (el, idx) {
          ...
          for (let i = 0; i < el.days.length; i++) {
            if (el.days[i]) {
              ...
              //                } else {
              //                  res.push(<td>{el.days[i].enterTime ? Time.format('HHMM', el.days[i].enterTime) : ''}-{el.days[i].exitTime ? Time.format('HHMM', el.days[i].exitTime) : ''},<br/>{el.days[i].elapsedTime}</td>);
              //                }
            } else {
              res.push(React.createElement(
                'td',
        ...
        let res = [];
        for (let i = 0; i < self.state.content[0].days.length; i++) {
          res.push(React.createElement(
            'th',
            null,
            i + 1
