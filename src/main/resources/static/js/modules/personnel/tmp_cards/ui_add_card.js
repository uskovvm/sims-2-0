  ...
  return React.createClass({
    getInitialState: function () {
      return {
        ...
        card: this.getEmptyItem(),
        doc: this.getEmptyDoc(),
        isDec: true
      };
    },
    getEmptyItem: function () {
      let card = Objects.clone(Objects.Card);
      delete card.id;
      card.validFrom = Math.floor(new Date().getTime() / 1000) * 1000;
      card.validTo = Math.floor(new Date().getTime() / 1000) * 1000;
      return card;
    },
    getEmptyDoc: function () {
      let doc = Objects.clone(Objects.VerificationDocument);
      delete doc.id;
      return doc;
    },
    ...
    onChangeStartDate: function (date) {
      ...
      if (date) {
        ...
        card.validFrom -= card.validFrom % 3600000;
        this.setState({ card: card });
      }
    },
    onChangeEndDate: function (date) {
      ...
      if (date) {
        ...
        card.validTo -= card.validTo % 3600000;
        this.setState({ card: card });
      }
    },
    onChangeStartDate: function (date) {
