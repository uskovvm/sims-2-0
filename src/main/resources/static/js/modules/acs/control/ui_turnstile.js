...
define(['jquery', 'react', 'react-dom', 'objects', 'utils/utils', 'core/socket', 'dao/core', 'core/auth'], function ($, React, ReactDOM, Objects, Utils, Socket, CoreDao, Auth) {
  ...
  return React.createClass({
    ...
    onClick: function (ev) {
      ...
      switch (st) {
        case 1:
          params.commandId = 'openOnce';
          params.params = {
            direction: 1
          };
          this.setState({ isOpenIn: true }, self.handleTimeout);
        ...
        case 2:
          ...
          this.setState({ isOpenOut: true }, self.handleTimeout);

          break;

        case 3:
          params.commandId = !this.props.data.device.directionOpenMask[0] ? 'keepOpen' : 'close';
          params.params = {
            direction: 1
          };
        ...
        case 4:
          params.commandId = !this.props.data.device.directionOpenMask[1] ? 'keepOpen' : 'close';
          params.params = {
            direction: 2
          };
    ...
    render: function () {
      let btnInClassName = 'btn-turnstiles1 btn-turnstiles-in' + (this.state.isOpenIn ? '' : ' closed') + (this.state.checked ? '' : ' disabled'),
          btnOutClassName = 'btn-turnstiles1 btn-turnstiles-out' + (this.state.isOpenOut ? '' : ' closed') + (this.state.checked ? '' : ' disabled'),
